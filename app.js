...
host: 'localhost',
user: 'root',
 password: 'Foullane1996@',//Ycode@2021
database: 'SAM'
});
db.connect((err) => {
    if (err) throw err;
...
app.get('/update-candidature/approuvee/:email', (req, res) => {
...
app.get('/update-candidature/rejetee/:email', (req, res) => {
  const email = req.params.email;
...
app.post('/event', upload.single('imgevent'), (req, res) => {
  ...
  db.query(query, [titre, description, date_debut, date_fin, lieu,imgevent], (err, result) => {
    if (err) {
      console.error('Erreur lors de l\'insertion des données :', err);
      return res.status(500).send('Erreur lors de l\'insertion des données.');
    }
...
// Route pour afficher les actualités
app.get('/modifactualite', (req, res) => {
  const query = 'SELECT * FROM actulaite'; // Requête SQL pour récupérer toutes les actualités
  db.query(query, (err, results) => {
      if (err) {
          console.error(err);
          return res.status(500).send('Erreur de la base de données');
      }
      res.render('modifactualite', { actualites: results }); // Transmettre les données à la vue
  });
});


// Suppression d'une actualité
app.post('/delete-actualite/:id', (req, res) => {
  const id = req.params.id;
  const query = 'DELETE FROM actulaite WHERE id = ?';
  db.query(query, [id], (err, result) => {
      if (err) throw err;
      res.redirect('/modifactualite'); // Redirection vers la page des actualités
  });
});

// Affichage du formulaire de modification
app.get('/edit-actualite/:id', (req, res) => {
  const id = req.params.id;
  const query = 'SELECT * FROM actulaite WHERE id = ?';
  db.query(query, [id], (err, result) => {
      if (err) throw err;
      res.render('editactualite', { actualite: result[0] });
  });
});

// Mise à jour d'une actualité
app.post('/update-actualite/:id', (req, res) => {
  const id = req.params.id;
  const { titre, contenu, image } = req.body;
  const query = 'UPDATE actulaite SET titre = ?, contenu = ?, image = ? WHERE id = ?';
  db.query(query, [titre, contenu, image, id], (err, result) => {
      if (err) throw err;
      res.redirect('/modifactualite'); // Redirection vers la page des actualités
  });
});
app.get('/modifevenement', (req, res) => {
  const query = 'SELECT * FROM evenements'; // La requête pour récupérer les événements
  ...
  db.query(query, (err, result) => {
      ...
      res.render('modifevenement', { evenements: result });
  });
});
// Route pour afficher le formulaire d'édition d'un événement
app.get('/edit-evenement/:id', (req, res) => {
  const evenementId = req.params.id;
  const query = 'SELECT * FROM evenements WHERE id = ?';
  
  db.query(query, [evenementId], (err, results) => {
      if (err) {
          console.error(err);
          return res.status(500).send('Erreur de la base de données');
      }
      ...
      res.render('editevenement', { evenement: results[0] });
  });
});

// Route pour mettre à jour un événement
app.post('/update-evenement/:id', (req, res) => {
  const evenementId = req.params.id;
  const { titre, description, date_debut, date_fin, lieu, image } = req.body;
  const query = 'UPDATE evenements SET titre = ?, description = ?, date_debut = ?, date_fin = ?, lieu = ?, image = ? WHERE id = ?';
  
  db.query(query, [titre, description, date_debut, date_fin, lieu, image, evenementId], (err) => {
      if (err) {
          console.error(err);
          return res.status(500).send('Erreur de la base de données');
      }
      res.redirect('/modifevenement');
  });
});

// Route pour supprimer un événement
app.post('/delete-evenement/:id', (req, res) => {
  const evenementId = req.params.id;
  const query = 'DELETE FROM evenements WHERE id = ?';
  
  db.query(query, [evenementId], (err) => {
      if (err) {
          console.error(err);
          return res.status(500).send('Erreur de la base de données');
      }
...
app.get('/index', (req, res) => {
  ...
  app.get('/candidater', (req, res) => {
   ...
   app.get('/modifactualite' , (req, res) => {
    ...
    res.render('modifactualite');
   });

   app.get('/modifevenement' , (req, res) => {
   
    res.render('modifevenement');
   });
